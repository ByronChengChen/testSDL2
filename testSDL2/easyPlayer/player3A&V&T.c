...
//这个文件对应的是 67.你必须清楚的多线程与锁 68.锁与条件变量的实际应用 69.播放器内核线程模型讲解 70.播放器核心结构体讲解 71.播放器核心代码逻辑讲解 player4.c
void packet_queue_init3(PacketQueue *q) {
    memset(q, 0, sizeof(PacketQueue));
    q->mutex = SDL_CreateMutex();
    q->cond = SDL_CreateCond();
}

int packet_queue_put3(PacketQueue *q, AVPacket *pkt) {
...
int packet_queue_get3(PacketQueue *q, AVPacket *pkt, int block)
{
    AVPacketList *pkt1;
    int ret;
...
int audio_decode_frame3(VideoState *is, uint8_t *audio_buf, int buf_size) {
    ...
    for(;;) {
        while(is->audio_pkt_size > 0) {
            ...
            if(got_frame) {
                ...
                fwrite(audio_buf, 1, data_size, audiofd3);
                fflush(audiofd3);
            }
        ...
        /* next packet */
        if(packet_queue_get3(&is->audioq, pkt, 1) < 0) {
            return -1;
        }
...
void audio_callback3(void *userdata, Uint8 *stream, int len) {
    ...
    while(len > 0) {
        if(is->audio_buf_index >= is->audio_buf_size) {
            ...
            audio_size = audio_decode_frame3(is, is->audio_buf, sizeof(is->audio_buf));
            if(audio_size < 0) {
...
void video_display3(VideoState *is) {
    ...
    if(vp->pict) {
        ...
        SDL_UpdateYUVTexture( texture3, NULL,
        ...
        SDL_LockMutex(texture_mutex3);
        SDL_RenderClear(renderer3);
        SDL_RenderCopy(renderer3, texture3, NULL, &rect);
        SDL_RenderPresent(renderer3);
        SDL_UnlockMutex(texture_mutex3);
        
    }
}

void video_refresh_timer3(void *userdata) {
...
void alloc_picture3(void *userdata) {
...
int queue_picture3(VideoState *is, AVFrame *pFrame) {
    ...
    if(is->quit){
        fprintf(stderr, "quit from queue_picture3....\n");
        return -1;
    }
    ...
    if(!vp->pict ||
       ...
       vp->height != is->video_ctx->height) {
        ...
        alloc_picture3(is);
        if(is->quit) {
            fprintf(stderr, "quit from queue_picture32....\n");
            return -1;
        }
    }
...
int video_thread(void *arg) {
    ...
    for(;;) {
        if(packet_queue_get3(&is->videoq, packet, 1) < 0) {
...
int stream_component_open3(VideoState *is, int stream_index) {
    ...
    if(codecCtx->codec_type == AVMEDIA_TYPE_AUDIO) {
        ...
        wanted_spec.callback = audio_callback3;
...
int decode_thread(void *arg) {
    ...
    if(audio_index >= 0) {
        stream_component_open3(is, audio_index);
    }
    if(video_index >= 0) {
        stream_component_open3(is, video_index);
    }
...
//                                is->video_ctx->height);
    ...
    for(;;) {
        ...
        if(packet->stream_index == is->videoStream) {
            packet_queue_put3(&is->videoq, packet);
            fprintf(stderr, "put video queue, size :%d\n", is->videoq.nb_packets);
        } else if(packet->stream_index == is->audioStream) {
            packet_queue_put3(&is->audioq, packet);
...
void initSDLVideo(){
    win3 = SDL_CreateWindow("Media Player",
                           SDL_WINDOWPOS_UNDEFINED,
                           SDL_WINDOWPOS_UNDEFINED,
                           854, 480,
                           SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
    renderer3 = SDL_CreateRenderer(win3, -1, 0);
    texture3 = SDL_CreateTexture(renderer3,
...
//视频，音频，线程
int playerA_V_T(int argc, char *argv[]) {
    argv[1] = "tsubasa.MP4";
    ...
    texture_mutex3 = SDL_CreateMutex();
    ...
    for(;;) {
        ...
        switch(event.type) {
            ...
            case FF_REFRESH_EVENT:
                ...
                video_refresh_timer3(event.user.data1);
                break;
            default:
                break;
        }
