...
//这个文件对应的是 65.实现最简单的播放器-3 66.实现最简单的播放器-4 对应李超的代码是 player3.c
...
void packet_queue_init2(PacketQueue *q) {
    memset(q, 0, sizeof(PacketQueue));
    q->mutex = SDL_CreateMutex();
    q->cond = SDL_CreateCond();
}

int packet_queue_put2(PacketQueue *q, AVPacket *pkt) {
...
int packet_queue_get2(PacketQueue *q, AVPacket *pkt, int block)
{
    AVPacketList *pkt1;
    int ret;
...
int audio_decode_frame2(AVCodecContext *aCodecCtx, uint8_t *audio_buf, int buf_size) {
    ...
    for(;;) {
        ...
        if(packet_queue_get2(&audioq, &pkt, 1) < 0) {
            return -1;
        }
...
int easyPlayer2(int argc, char *argv[]) {
    argv[1] = "tsubasa.MP4";
    int  		  ret = -1;
    ...
    while(av_read_frame(pFormatCtx, &packet)>=0) {
        ...
        if(packet.stream_index==videoStream) {
            ...
            packet_queue_put2(&audioq, &packet);
        } else {
            av_free_packet(&packet);
        }
